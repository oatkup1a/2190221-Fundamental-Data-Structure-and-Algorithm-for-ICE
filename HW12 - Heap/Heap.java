public class Heap {
	...
	public static boolean isAHeap(Heap h) {
		//loop as every element becomes parent
	    for (int parent=0;parent < h.size;parent++) {
	        int leftChild = 2 * parent + 1;
	        int rightChild = 2 * parent + 2;
	        
	        //Check left and right child & both left and right must still be in the array
	        if ( (leftChild < h.size && h.mData[parent] > h.mData[leftChild]) || (rightChild < h.size && h.mData[parent] > h.mData[rightChild]) )
	            return false;
	    }
	    return true;
	}
