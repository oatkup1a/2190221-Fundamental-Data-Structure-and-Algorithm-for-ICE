...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		// if emptyTree, move root and add size
		if (this.isEmpty()) {
			this.root = subtree.root;
			this.size += subtree.size;
		}
		
		//If highest value of subtree is more than lowest value of this, do nothing
		if (((TreeIterator)subtree.findMax()).currentNode.data > ((TreeIterator)this.findMin()).currentNode.data) {
			return;
		}
		
		//left node of lowest value = root of subtree and parent of subtree = lowest value node
